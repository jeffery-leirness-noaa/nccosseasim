...
run_simulation <- function(
  ...
  #   m_samp <- samp_str |>
  #     fit_comp_dirichlet(formula = formula, verbose = verbose)
  ...
  sim <- sim |>
    SimEngine::set_script(
      function() {
        ...
        m_samp <- samp_str |>
          nccosseasim::fit_comp_dirichlet(formula = formula, verbose = verbose)
        pred_samp <- nccosseasim::predict_comp_dirichlet(
          sim_data,
          model = m_samp
        ) |>
          terra::unwrap()
