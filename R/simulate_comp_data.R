...
simulate_comp_data <- function(
  ...
  form_base <- stringr::str_flatten(layer_names, collapse = " + ") |>
    pipebind::bind(._, stringr::str_c("1 + ", ._))
  form_components <- rep(list(form_base), d)
  form_sim <- form_components |>
  ...
  x_sim <- list()
  eta_sim <- matrix(NA, nrow = nrow(df), ncol = d)
  for (i in 1:d) {
    ...
    x_sim[[i]] <- stats::rnorm(ncol(design_matrix), mean = 0, sd = 2)
    eta_sim[, i] <- design_matrix %*% x_sim[[i]]
  }
